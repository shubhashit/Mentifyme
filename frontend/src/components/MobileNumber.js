...
import arrowleftsquare from '../assets/arrowleftsquare.svg'
...
export default function MobileNumber() {
    ...
    return (
        <div>
            ...
            <div>
                <div className='m-6 '>
                    <div className='text-2xl font-bold'>Your Mobile Number?</div>
                </div>
                <div className='m-6 flex-row flex mt-20 border-b border-[#696DCA] w-[40vw] justify-between items-center'>
                    <input type="text" className='outline-none w-full  text-[#696DCA] text-2xl' placeholder='Enter Your Mobile Number(Whatsapp)  Here' />
                    <div><img src={arrowleftsquare} className='cursor-pointer' alt="" /></div>
                </div>
            </div>
            <div className='m-6 w-[50vw] flex flex-row justify-between mt-10 h-12 '>
                <button className='bg-[#696DCA] pr-4 pl-4 pt-1 pb-1 text-lg text-white rounded-sm mr-3 w-1/5 flex flex-row items-center' onClick={() => { navigate("/timetable/Name") }}> <img src={back} alt="" /> Previours</button>
                <button className='bg-[#696DCA] pr-4 pl-4 pt-1 pb-1 text-lg text-white rounded-sm mr-3 w-1/5 flex flex-row justify-center items-center opacity-50' onClick={() => { navigate("/timetable/ExamType") }}>Next <img src={forward} alt="" /></button>
            </div>
        </div>
    )
}
