...
export default function TypeOfperson() {
    ...
    return (
        <div>
            ...
            <div>
                <div className='m-6 '>
                    <div className='text-2xl font-bold'>At what time your generally Wakeup?</div>
            ...
            <div className='m-6 w-[50vw] flex flex-row justify-between mt-10 h-12 '>
                <button className='bg-[#696DCA] pr-4 pl-4 pt-1 pb-1 text-lg text-white rounded-sm mr-3 w-1/5 flex flex-row items-center' onClick={() => { navigate("/timetable/TypeOfperson") }}> <img src={back} alt="" /> Previours</button>
                <button className='bg-[#696DCA] pr-4 pl-4 pt-1 pb-1 text-lg text-white rounded-sm mr-3 w-1/5 flex flex-row justify-center items-center opacity-50' onClick={() => { navigate("/timetable/SleepTime") }}>Next <img src={forward} alt="" /></button>
            </div>
        </div>
    )
}
