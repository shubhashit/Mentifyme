...
export default function Signup(props) {
    ...
    return (
        <>
           {page == 1 ? <div className={`h-[100vh] w-screen md:w-[70vw] ${dark ? 'bg-black' : 'bg-white'} absolute right-0 top-0 md:pl-12 p-4`}>
                <div className='relative flex justify-end '><img src={dark ? xwhite : x} className='m-4 cursor-pointer' alt="" onClick={() => { navigate("/") }} /></div>
                <div className='mt-12'>

                    <div className='text-2xl md:text-5xl font-bold text-[#696DCA] mb-2'  >Signup</div>
                    <div className='text-[#696DCA] font-semibold text-base md:text-lg mb-4 cursor-pointer' onClick={Tologin}>or log into your account</div>
                    <div className={`border-2 outline-none ${dark ? "border-white text-white" : "border-[#696DCA]"} w-full md:w-3/5 p-4 text-base md:text-xl mb-4 flex flex-row  items-center`}>
                        +91 |  <span><input ref={InputNum} type="text" placeholder='9039144565' className={`outline-none ml-1 ${dark ? "bg-black placeholder:text-white text-white" : "bg-white placeholder:text-[#696DCA]"} text-sm md:text-base`} /></span>
                    </div>
                    <div><button className='bg-[#696DCA]  p-6 pt-3 pb-3 w-full md:w-[10rem] text-xl text-white rounded-md mr-3 mb-4 md:mb-10 mt-4 md:mt-8' onClick={Onsignup}>Signup</button></div>
